...
Choose this focused tool when your immediate task is to create three entry points with one common mastery product. Use the task-specific check below to distinguish it from related tools.
...
```text
Act as a teacher-facing drafting assistant. Task DF-014: Create three entry points with one common mastery product.
...
Grade 5 mathematics; compare 3/4 and 5/6 using equal wholes; paper strips available; 20 minutes; observed barrier: organizing a written explanation. This is an intentionally incomplete input-check exercise for DF-014, not a complete example run. Identify which of ENTRY_BARRIERS, COMMON_PRODUCT are still needed. Expected behavior: request missing essentials, not invent the finished artifact.

## Teacher verification checklist

- [ ] Task-specific acceptance: Design three starting activities that converge on the same final task. Include a bridge so entry support does not become a separate curriculum.
