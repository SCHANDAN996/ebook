...
Choose this focused tool when your immediate task is to tier a task by representation rather than by easier content. Use the task-specific check below to distinguish it from related tools.
...
```text
Act as a teacher-facing drafting assistant. Task DF-013: Tier a task by representation rather than by easier content.
...
Grade 5 mathematics; compare 3/4 and 5/6 using equal wholes; paper strips available; 20 minutes; observed barrier: organizing a written explanation. This is an intentionally incomplete input-check exercise for DF-013, not a complete example run. Identify which of ALLOWED_REPRESENTATIONS, CORE_CONCEPT are still needed. Expected behavior: request missing essentials, not invent the finished artifact.

## Teacher verification checklist

- [ ] Task-specific acceptance: Offer concrete, visual and symbolic routes. Verify that each requires the same conceptual decision rather than merely different decoration.
