...
Choose this focused tool when your immediate task is to write concise comments within a strict character limit. Use the task-specific check below to distinguish it from related tools.
...
```text
Act as a teacher-facing drafting assistant. Task RC-007: Write concise comments within a strict character limit.
...
Shorten to the exact limit while preserving concern and next step. Include a character count and verify it locally before use.
...
Grade 6 mathematics; fictional record: three of four equivalent-fraction items correct; one explanation missing; 70-word limit; no identity supplied. This is an intentionally incomplete input-check exercise for RC-007, not a complete example run. Identify which of COMMENT, CHARACTER_LIMIT are still needed. Expected behavior: request missing essentials, not invent the finished artifact.

## Teacher verification checklist

- [ ] Task-specific acceptance: Shorten to the exact limit while preserving concern and next step. Include a character count and verify it locally before use.
