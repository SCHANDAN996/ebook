...
Choose this focused tool when your immediate task is to build a single-point rubric with feedback space. Use the task-specific check below to distinguish it from related tools.
...
```text
Act as a teacher-facing drafting assistant. Task AS-023: Build a single-point rubric with feedback space.
...
Grade 8 mathematics; solve and justify 2x+3=11; correct answer x=4; paper responses; ten minutes. This is an intentionally incomplete input-check exercise for AS-023, not a complete example run. Identify which of SUCCESS_CRITERIA, FEEDBACK_SPACE are still needed. Expected behavior: request missing essentials, not invent the finished artifact.

## Teacher verification checklist

- [ ] Task-specific acceptance: Create a single-point rubric with target descriptors and separate evidence below/beyond target. Do not disguise it as a four-level scale.
